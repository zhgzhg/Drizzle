...
For e.g. `java -jar drizzle-0.16.1-with-deps.jar --parse hello-world.ino` will produce:
...
```
{
  ...
  "arduino_ide_tools": {
    "Drizzle": {
      "version": "(<0.16.1)",
      "url": "https://github.com/zhgzhg/Drizzle/releases/download/0.16.1/drizzle-0.16.1-dist.zip"
    }
  }
}
```

Executing on the above JSON `java -jar drizzle-0.16.1-with-deps.jar --rev-parse hello-world.json` will produce:
